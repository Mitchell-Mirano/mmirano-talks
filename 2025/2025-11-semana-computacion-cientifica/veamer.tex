...
\begin{itemize}
  ...
  \[
  \hat{y} = z = \sum_{i=1}^{n} w_i x_i + b = \mathbf{w}^\top \mathbf{x} + b
  \]
...
donde:
\begin{itemize}
  \item \(\mathbf{X} \in \mathbb{R}^{n \times m}\): matriz de diseño o entradas.
...
\[
\mathcal{L}(\mathbf{w}, b) = \frac{1}{n} \| \mathbf{X}\mathbf{w} + \mathbf{b} - \mathbf{y} \|_2^2
\]
...
\[
\frac{\partial \mathcal{L}}{\partial w_j} = \frac{2}{n} \sum_{i=1}^{n} e_i \, \mathbf{X}_{ij}
\qquad
...
\[
\nabla_{\mathbf{w}} \mathcal{L} = \frac{2}{n} \mathbf{X}^\top (\mathbf{X}\mathbf{w} + \mathbf{b} - \mathbf{y})
\]
\[
\frac{\partial \mathcal{L}}{\partial b} = \frac{2}{n} \mathbf{1}_n^\top (\mathbf{X}\mathbf{w} + \mathbf{b} - \mathbf{y})
\]

Donde:
\begin{itemize}
  \item \(\mathbf{1}_n\) es el vector de unos en \(\mathbb{R}^n\).
\end{itemize}
...
% \[
% X^\top(\mathbf{X}\mathbf{w} + \mathf{b} - \mathbf{y}) = 0
% \]
% \[
% \mathbf{1}_n^\top (\mathbf{X}\mathbf{w} + \mathf{b} - \mathbf{y}) = 0
% \]

\[ 
\mathbf{X}^\top(\mathbf{X}\mathbf{w} + \mathbf{b} - \mathbf{y}) = 0
\]

\[
\mathbf{1}_n^\top (\mathbf{X}\mathbf{w} + \mathbf{b} - \mathbf{y}) = 0
\]
...
\[
\begin{cases}
\mathbf{X}^\top \mathbf{X}\mathbf{w} + b \mathbf{X}^\top = \mathbf{X}^\top \mathbf{y} \\
\mathbf{1}_n^\top \mathbf{X} \mathbf{w} + b\mathbf{1}_n^\top = \mathbf{1}_n^\top \mathbf{y}
\end{cases}
\]

\textbf{Solución cerrada para los parámetros:}
\[
\boxed{\mathbf{w}^* = \mathbf{(X^\top H X)^{-1} X^\top H}\mathbf{y}}
\]
\[
...
\textbf{Donde:}
\[
\mathbf{H} = I_n - \frac{1}{n}\mathbf{1}_n \mathbf{1}_n^\top, \qquad
\bar{\mathbf{x}} = \frac{1}{n} \mathbf{X}^\top \mathbf{1}_n \in \mathbb{R}^m, \qquad
...
\textbf{Objetivo:} minimizar una función de costo diferenciable \(\mathcal{L}(\mathbf{w}, b)\) ajustando iterativamente los parámetros.
...
\[
\mathbf{w}^{(t+1)} = \mathbf{w}^{(t)} - \frac{2\eta}{n} \mathbf{X}^\top (\mathbf{X}\mathbf{w}^{(t)} + \mathbf{b}^{(t)} - \mathbf{y})
\]
\[
b^{(t+1)} = b^{(t)} - \frac{2\eta}{n} \mathbf{1}_n^\top (\mathbf{X}\mathbf{w}^{(t)} + \mathbf{b}^{(t)} - \mathbf{y})
\]
...
\[
\mathbf{W} \in \mathbb{R}^{m \times k}, \qquad \mathbf{B} \in \mathbb{R}^{nxk}.
\]
